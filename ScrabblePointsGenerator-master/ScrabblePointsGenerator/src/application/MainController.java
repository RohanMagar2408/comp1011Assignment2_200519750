...
public class MainController implements Initializable {
	...
	public void PrevWordsPage(ActionEvent event) throws IOException {
		Stage window=(Stage)((Node)event.getSource()).getScene().getWindow();
		window.setScene(Main.getPrevWordsPage());
		window.show();	
	}
	...
	private boolean verify(String text) {
		...
		if(text==null||text.length()<2||text.length()>8) {
			error.setText("Error: word length in range [2-8]");
			return false;
		}
		...
		if(arr['A'-'A']+arr['E'-'A']+arr['I'-'A']+arr['O'-'A']+arr['U'-'A']+arr['Y'-'A']==0) {
			error.setText("Error:  One letter must be vowel (A, E, I, O, U) or Y!");
			return false;
		}
		if(!model.CheckPoints(arr)) {
			error.setText("Error:  not enough points for this word!");
			return false;
		}
